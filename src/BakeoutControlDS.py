...
#------------------------------------------------------------------------------ 
    def init_device(self):
        ...
        try: 
            ...
            raise Exception("AttributeProxyError")
        try:
            if ( self.ControllerType.lower() == "eurotherm" ):
                ...
                self._c.start()
                self._q.put((0, ControllerCommand.STOP))                
            else:
                raise Exception("UnknownController: %s" % self.ControllerType)
        except Exception, e:
#            self._modbus = None
    ...
    def CheckStatus(self):
        ...
        for zone in range(1, self.zoneCount() + 1):
            ...
            if ( st ):
                statusStr += " ON"
                if ( pr ):
                    statusStr += " and RUNNING program %d." % pr
                else:
                    statusStr += " and NOT RUNNING any program. ALARM!"
                    alarm = True
            else:
                statusStr += " OFF"
            statusStr += "\n"
...
#------------------------------------------------------------------------------ 
    def Start(self, programNo):
        ...
        if ( not self.controller().isRunning(programNo) ):
...
#------------------------------------------------------------------------------ 
    def Stop(self, zone):
        ...
        if ( self.controller().isRunning(zone) ):
...
#    ramp()        
    ...
    def run(self):
        ...
        while ( not self.isFinished() ):
            ...
            temp = self.temp()
            ramp = self.ramp()
            timeout = 60. * (self.time() + abs(self.startTemp() - temp) / ramp)
